...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "verack")
    {
        ...
        CTxOut out;
        CTxOut out2;
        vRecv >> in >> nAmount >> txCollateral >> txSupporting >> out >> out2;
        ...
        std::string strMessage = addr.ToString() + boost::lexical_cast<std::string>(sigTime) + pubkey2.ToString(); 
    ...
    if(state == POOL_STATUS_FINALIZE_TRANSACTION && finalTransaction == CTransaction()) {
        ...
        if (fMasterNode) { 
            ...
            for(unsigned int i = 0; i < entries.size(); i++){
                txNew.vout.push_back(entries[i].vout);
                txNew.vout.push_back(entries[i].vout2);
                txNew.vin.push_back(entries[i].vin);
            }
...
std::string CDarkSendPool::Denominate(){
    CWalletTx d = CWalletTx();
    return pwalletMain->Denominate(d);
}
...
bool CDarkSendPool::SignatureValid(const CScript& newSig, const CTxIn& newVin){
    ...
    for(unsigned int i = 0; i < entries.size(); i++){
        ...
        txNew.vout.push_back(entries[i].vout2);
        txNew.vin.push_back(entries[i].vin);
        if(entries[i].vin == newVin){
            found = i;
            sigPubKey = entries[i].sigPubKey;
        }
    }
...
bool CDarkSendPool::AddEntry(const CTxIn& newInput, const int64& nAmount, const CTransaction& txCollateral, const CTxOut& newOutput, const CTxOut& newOutput2, std::string& error){
    ...
    if(state == POOL_STATUS_ACCEPTING_ENTRIES) {
        CDarkSendEntry v;
        v.Add(newInput, nAmount, txCollateral, newOutput, newOutput2);
        entries.push_back(v);
...
bool CDarkSendPool::AddScriptSig(const CScript& newSig, const CTxIn& newVin, const CScript& newPubKey){
    printf("CDarkSendPool::AddScriptSig -- new sig  %s\n", newSig.ToString().substr(0,24).c_str());
    ...
    if(!SignatureValid(newSig, newVin)){
        printf("CDarkSendPool::AddScriptSig - Invalid Sig\n");
        return false;
    }
    ...
    if(state == POOL_STATUS_SIGNING) {
        for(unsigned int i = 0; i < entries.size(); i++){
            if(entries[i].vin == newVin){
                ...
                entries[i].AddSig(newSig, newPubKey);
                sigCount++;
...
void CDarkSendPool::SendMoney(const CTransaction& collateral, CTxIn& in, CTxOut& out, int64& fee, int64 amount, CScript& pubScript, const CTransaction& txSupporting){
    ...
    CTxOut change = CTxOut(amount-out.nValue-fee, scriptChange);
    
    in.prevPubKey = pubScript;
...
bool CDarkSendPool::SignFinalTransaction(CTransaction& finalTransactionNew, CNode* node){
    ...
    BOOST_FOREACH(const CDarkSendEntry e, myEntries) {
        ...
        if(mine >= 0){ //might have to do this one input at a time?
            ...
            AddScriptSig(finalTransaction.vin[n].scriptSig, e.vin, e.vin.prevPubKey);
...
void CDarkSendPool::RegisterAsMasterNode(bool stop)
{
    ...
    if(isCapableMasterNode == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vinMasterNode, pubkeyMasterNode, SecretKey)) {
            ...
            std::string strMessage = addr.ToString() + boost::lexical_cast<std::string>(masterNodeSignatureTime) + MasterNodeSigningPubKey.ToString();
            ...
            if(!found) {                
                ...
                CMasterNode mn(addr, vinMasterNode, pubkeyMasterNode, vchMasterNodeSignature, masterNodeSignatureTime, MasterNodeSigningPubKey);
                mn.UpdateLastSeen();
                darkSendMasterNodes.push_back(mn);
                printf("CDarkSendPool::RegisterAsMasterNode() - Masternode input = %s\n", vinMasterNode.ToString().c_str());
            }
        
            RelayDarkSendElectionEntry(vinMasterNode, addr, vchMasterNodeSignature, masterNodeSignatureTime, pubkeyMasterNode, MasterNodeSigningPubKey, -1, -1);
    ...
    masterNodeSignatureTime = GetTimeMicros();
    std::string strMessage = addr.ToString() + boost::lexical_cast<std::string>(masterNodeSignatureTime) + MasterNodeSigningPubKey.ToString() + boost::lexical_cast<std::string>(stop);

    if(!darkSendSigner.SignMessage(strMessage, errorMessage, vchMasterNodeSignature, MasterNodeSigningKey)) {
        printf("CDarkSendPool::RegisterAsMasterNode() - Sign message failed");
        return;
    }

    if(!darkSendSigner.VerifyMessage(MasterNodeSigningPubKey, vchMasterNodeSignature, strMessage, errorMessage)) {
        printf("CDarkSendPool::RegisterAsMasterNode() - Verify message failed");
        return;
    }
    ...
    if (stop) isCapableMasterNode = MASTERNODE_IS_STOPPED
...
void CDarkSendPool::SetMasterNodePrivKey(Key key, CPubKey pubkey)
{
    MasterNodeSigningKey = key;
    MasterNodeSigningPubKey = pubkey;
}
