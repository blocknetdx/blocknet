...
UniValue dxMakeOrder(const JSONRPCRequest& request)
{
    ...
    * This only applies to `partial` order types and will show `0` on `exact`
    ...
    statusCode = xbridge::App::instance().sendXBridgeTransaction(fromAddress, fromCurrency, fromAmount,
                                                                 toAddress, toCurrency, toAmount, 
                                                                 id, blockHash);
...
UniValue dxMakePartialOrder(const JSONRPCRequest& request)
{
    ...
    * This only applies to `partial` order types and will show `0` on `exact`
    ...
    statusCode = xbridge::App::instance().sendXBridgeTransaction(fromAddress, fromCurrency, fromAmount, 
                                                                 toAddress, toCurrency, toAmount, 
                                                                 std::vector<xbridge::wallet::UtxoEntry>{},
                                                                 true, repost, partialMinimum, 
                                                                 id, blockHash);
