...
bool Session::Impl::processTransaction(XBridgePacketPtr packet) const
{
    ...
    if (t->matches(id)) 
    {
        ...
        {
            if (!e.makerUtxosAreStillValid(t)) 
            { 
                // if the maker utxos are no longer valid, cancel the order
...
bool Session::Impl::processTransactionAccepting(XBridgePacketPtr packet) const
{
    ...
    //    token heights. This lowers risk of pairing counterparies on forks
    {
        ...
        if (conn->getBlockCount(sourceBlockHeightSnode) && 
            conn->getBlockHash(sourceBlockHeightSnode, sourceBlockHashSnode) && 
            connDest->getBlockCount(destinationBlockHeightSnode) && 
            connDest->getBlockHash(destinationBlockHeightSnode, destinationBlockHashSnode) && 
            conn->getBlockHash(sourceHeight, sourceBlockHashCounterparty) && 
...
bool Session::Impl::processTransactionCreateA(XBridgePacketPtr packet) const
{
    ...
    if (xtx->isPartialRepost()) 
    {
        ...
        try 
        {
            const auto status = xapp.repostXBridgeTransaction(xtx->fromAddr, xtx->fromCurrency, 
                                                                xtx->toAddr, xtx->toCurrency,
                                                                xtx->origFromAmount, xtx->origToAmount, 
                                                                xtx->minFromAmount, xtx->repostCoins, xtx->id);
...
bool Session::Impl::processTransactionCreateB(XBridgePacketPtr packet) const
{
    ...
    amount_t fee1      = 0;
    amount_t fee2      = connFrom->minTxFee2(1, 1);
    amount_t inAmount  = 0;
...
bool Session::Impl::processTransactionConfirmB(XBridgePacketPtr packet) const
{
    ...
    std::string payTxId;
    offset += packet->read(offset, payTxId);
...
bool Session::Impl::processTransactionConfirmedB(XBridgePacketPtr packet) const
{
    ...
    // Pay tx id from B
    std::string b_payTxId;
    offset += packet->read(offset, b_payTxId);
...
bool Session::Impl::processTransactionCancel(XBridgePacketPtr packet) const
{
    ...
    TxCancelReason reason = static_cast<TxCancelReason>(*reinterpret_cast<uint32_t*>(packet->data() + offset));
    ...
    else if (xtx->state < TransactionDescr::trCreated) 
    { 
        // if no deposits yet
    ...
    else if (!xtx->didSendDeposit()) 
    { 
        // cancel if deposit not sent
        cancel(log_obj, "counterparty cancel request");
        return true;
    } 
    else if (xtx->hasRedeemedCounterpartyDeposit()) 
    { 
        // Ignore if counterparty deposit already redeemed
...
bool Session::Impl::processTransactionFinished(XBridgePacketPtr packet) const
{
    ...
    { 
        ...
        if (!packet->verify(xtx->sPubKey)) 
        { 
            // ignore packet if not from snode
            UniValue log_obj(UniValue::VOBJ);
            log_obj.pushKV("orderid", txid.GetHex());
            log_obj.pushKV("expected_snode", HexStr(xtx->sPubKey));
            log_obj.pushKV("received_snode", HexStr(spubkey));
...
bool Session::Impl::redeemOrderDeposit(const TransactionDescrPtr & xtx, int32_t & errCode) const 
{
    ...
    if (xtx->refTx.empty()) 
    {
        ...
        if (!xtx->binTx.empty()) 
        { 
            // if there's a bin tx but no rollback, could mean potential loss of funds
            UniValue log_obj(UniValue::VOBJ);
            log_obj.pushKV("orderid", txid.GetHex());
