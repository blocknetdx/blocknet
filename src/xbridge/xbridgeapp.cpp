...
void App::Impl::onSend(const std::vector<unsigned char> & id, const std::vector<unsigned char> & message)
{
    ...
    int256_t timestampValue = timeToInt(timestamp);
    unsigned char * ptr = reinterpret_cast<unsigned char *>(&timestampValue);
    msg.insert(msg.end(), ptr, ptr + sizeof(int256_t));
...
void App::updateActiveWallets()
{
    ...
    for (std::vector<std::string>::iterator i = wallets.begin(); i != wallets.end(); ++i)
    {
        ...
        wp.COIN                        = s.get<int256_t>   (*i + ".COIN", 0);
        wp.txVersion                   = s.get<uint32_t>   (*i + ".TxVersion", 1);
        wp.minTxFee                    = s.get<int256_t>   (*i + ".MinTxFee", 0);
        wp.feePerByte                  = s.get<int256_t>   (*i + ".FeePerByte", 0);
...
xbridge::Error App::repostXBridgeTransaction(const std::string from, const std::string fromCurrency,
        ...
        const int256_t minFromAmount, const std::vector<wallet::UtxoEntry> utxos, const uint256 parentid)
{
...
Error App::acceptXBridgeTransaction(const uint256 & id, const std::string & from, const std::string & to,
                                    const int256_t fromSize, const int256_t toSize)
{
...
//******************************************************************************
//******************************************************************************
Error App::checkAcceptParams(const std::string fromCurrency, const int256_t fromAmount) {
