...
bool CActiveServicenode::Register(std::string strService, std::string strKeyServicenode, std::string strTxHash, std::string strOutputIndex, std::string& errorMessage)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        if (service.GetPort() != 51472) {
            errorMessage = strprintf("Invalid port %u for servicenode %s - only 51472 is supported on mainnet.", service.GetPort(), strService);
            LogPrintf("CActiveServicenode::Register() - %s\n", errorMessage);
            return false;
        }
    } else if (service.GetPort() == 51472) {
        errorMessage = strprintf("Invalid port %u for servicenode %s - 51472 is only supported on mainnet.", service.GetPort(), strService);
        LogPrintf("CActiveServicenode::Register() - %s\n", errorMessage);
        return false;
    }
