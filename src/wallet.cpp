...
bool CWallet::GetDarksendSubsciption(CTransaction& tx, std::vector<unsigned char>& vchSig, CTransaction& txSupporting)
{
...

    {
        ...
        {
            const CWalletTx* pcoin = &(*it).second;
            if(pcoin->GetDepthInMainChain() < DARKSEND_SUBSCRIPTION_MAX){//we have an active subscription
                int64 nValueIn = 0;
                int64 nValueOut = 0;
                bool missingTx = false;

                BOOST_FOREACH(const CTxOut o, pcoin->vout)
                    nValueOut += o.nValue;

                if(pcoin->vin.size() > 1) continue; //not supported

                CWalletTx wtx;
                if(GetTransaction(pcoin->vin[0].prevout.hash, wtx)){
                    if(wtx.vout.size() > pcoin->vin[0].prevout.n) {
                        nValueIn += wtx.vout[pcoin->vin[0].prevout.n].nValue;    
                    }
                } else{
                    missingTx = true;
                }
                ...
                CScript pubkey = txSupporting.vout[tx.vin[0].prevout.n].scriptPubKey;

                CTxDestination address1;
                ExtractDestination(pubkey, address1);
                CBitcoinAddress address2(address1);

                if (!address2.IsValid()) continue;

                CKeyID keyID;
                if (!address2.GetKeyID(keyID)) continue;

                CKey key;
                if (!pwalletMain->GetKey(keyID, key)) continue;
                ...
                {
                    LogPrintf("CWallet::GetDarksendSubsciption - Can't sign my input\n");
                    continue;
                }

                LogPrintf("CWallet::GetDarksendSubsciption - success - %s \n", wtx.GetHash().ToString().c_str());
                return true;
            }
        }
    }
