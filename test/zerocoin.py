...
class ZerocoinTest(TestCaseBase):
    ...
    def test_wallet_tutorial(self):
        ...
        balance2 = self.nodes[1].getbalance()
        balance3 = self.nodes[2].getbalance()
        print("Node 0 spending 4 zerocoin...",)
        self.nodes[0].spendzerocoin(4, False, False, 100)
        self.nodes[0].setgenerate(True, 30)
        self.sync_all()
        assert_greater_than(self.nodes[0].getbalance(), balance1)
        print("Success!")
        print("Minting 280 blocks to get to 500")
        self.nodes[0].setgenerate(True, 280)
        self.sync_all()
        print("Attempting to spend after accumulator params change")
        self.nodes[1].spendzerocoin(4, False, False, 100)
